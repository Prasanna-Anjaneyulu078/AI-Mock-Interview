...
const fallbackInterviews = new Map();

const isDatabaseAvailable = () => mongoose.connection.readyState === 1;

const createFallbackInterview = (userId, role, resumeText, questions, greeting) => {
  const interview = {
    _id: `local-${Date.now()}`,
    userId,
    role,
  ...
  fallbackInterviews.set(interview._id, interview);
  return interview;
};
...
export const startInterview = async (userId, role, resumeText, candidateName, totalQuestions = 5) => {
  ...
  try {
    ...
    console.error('Interview question generation failed, using fallback questions:', error.message);
    aiQuestions = [
      {
        text: `Tell me about your experience with ${role || 'this role'} and how it relates to your resume.`,
        type: 'behavioral',
        isCodeQuestion: false,
      },
      {
        text: `Describe a project or accomplishment from your background that demonstrates relevant skills for ${role || 'this role'}.`,
        type: 'behavioral',
        isCodeQuestion: false,
      },
    ];
  ...
  let greeting = `Hi ${candidateName || 'there'}! Thanks for joining this ${role || 'interview'} session. I’ll ask a few questions based on your background and resume.`;
  try {
  ...
  if (!isDatabaseAvailable()) {
    const interview = createFallbackInterview(userId, role, resumeText, questions, greeting);
    return {
